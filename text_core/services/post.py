...
class PostService:
    ...
    def generate_post(self, input_prompt: InputPrompt) -> Post:
        if not input_prompt.prompt:
            target = prompts.items[input_prompt.category.name]["target"]
            audience = prompts.items[input_prompt.category.name]["audience"]
            input_prompt.prompt = f"{target} {audience}"

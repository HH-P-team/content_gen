...
from db.base_storage import BaseUserStore, BaseAccStore
from db.pg_user_tools import PgUserStoreTools
from db.pg_acc_tools import PgAccStoreTools
...
class UserService:
    ...
    async def get_user_by_uuid(self, uuid: UUID) -> Users:
        user = await self.user_db.get_user_by_uuid(uuid)
        return user

    async def update_user(self, user_uuid: UUID, **kwargs) -> Users:

        user = await self.user_db.update_user(user_uuid, **kwargs)
        return user

    async def acc_login(self, user_uuid: UUID):
        acc = await self.acc_db.add_acc(user_uuid)
        if acc:
            return acc.login_time

    async def get_acc_info(self, user_uuid: UUID):
...
def get_user_service(
    ...
    acc_db: BaseAccStore = Depends(PgAccStoreTools),
) -> UserService:

    return UserService(user_db, acc_db)
